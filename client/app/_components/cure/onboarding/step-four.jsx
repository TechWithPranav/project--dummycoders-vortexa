...
const StepFour = ({ nextStep }) => {
    ...
    const { mutate, isPending } = useMutation({
        ...
        mutationFn: async ({ anxiety, happiness, mood, sleep, stress }) => {
            const payload = {
                anxiety,
                happiness,
                mood,
                            ...
                            <FormItem>
                                ...
                                <RadioGroup value={field.value} onChange={field.onChange} className="grid grid-cols-2 md:grid-cols-6 gap-2">
                                    {["HAPPY", "SAD", "ANGRY", "ANXIOUS", "STRESSED", "NEUTRAL"].map((mood) => (
                                        ...
                                        >
                                            <RadioGroup.Label as="span" htmlFor={mood} className="text-sm !capitalize">
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <RadioGroup value={field.value} onChange={field.onChange} className="grid grid-cols-2 md:grid-cols-5 gap-2">
                                    {["NOT_STRESSED", "SLIGHTLY", "MODERATELY", "HIGHLY", "EXTREMELY"].map((stress) => (
                                        <RadioGroup.Option
                                            key={stress}
                                            value={stress}
                                            id={stress}
                                        ...
                                        >
                                            <RadioGroup.Label as="span" htmlFor={stress} className="text-sm !capitalize">
                                                {stress.replace("_", " ").toLowerCase()}
                                            </RadioGroup.Label>
                                        </RadioGroup.Option>
                                    ))}
                                </RadioGroup>
                            ...
                            <FormItem>
                                ...
                                <RadioGroup value={field.value} onChange={field.onChange} className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    {["GOOD", "BAD", "AVERAGE"].map((sleep) => (
                                        ...
                                        >
                                            <RadioGroup.Label as="span" htmlFor={sleep} className="text-sm !capitalize">
                                                {sleep.toLowerCase()}
                                            </RadioGroup.Label>
                                        </RadioGroup.Option>
                                    ))}
