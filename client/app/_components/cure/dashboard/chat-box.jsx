...
import { createMessages } from "@/app/_actions";
import ai from "@/app/_lib/google";
import { generatePrompt } from "@/app/_utils";
...
const ChatBox = ({ isPro, user, symptoms, medications, messages }) => {
    ...
    const { mutate: createUserMessage } = useMutation({
        ...
        mutationFn: async (message) => createMessages({ role: "user", message: message }),
    });

    const { mutate: createBotMessage } = useMutation({
        mutationKey: ["create-bot-messages"],
        mutationFn: async (message) => createMessages({ role: "model", message: message }),
    });
