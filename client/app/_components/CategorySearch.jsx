...
function CategorySearch() {
  ...
  return (
    <div className='mb-10 items-center px-5 flex flex-col gap-2 mt-10'>
      ...
      <Button type="submit">
        ...
        <div className='grid grid-cols-3 mt-5 md:grid-cols-4 lg:grid-cols-5 '>
        {categoryList.length>0?categoryList.map((item,index)=>index<20&&(
          <Link href={'/search/'+item.attributes.Name} key={index} className='flex 
          flex-col text-center items-center
          p-5 bg-green-50 m-2 rounded-lg cursor-pointer
          gap-2 hover:scale-110 transition-all ease-in-out'>
            <Image src={item.attributes?.Icon?.data.attributes?.url}
            alt='icon'
            width={40}
            height={40}/>
            <label className='text-green-600 text-sm'>{item?.attributes?.Name}</label>
          </Link>
        ))
      :
