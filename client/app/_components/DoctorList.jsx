...
function DoctorList({doctorList,heading='🧑‍⚕️ Meet our Doctors🩺 Here..'}) {
  return (
            ...
            {doctorList.length>0?doctorList.map((doctor,index)=>(
                ...
                key={index}>
                    ...
                    <div className='mt-3 items-baseline flex flex-col gap-1'>
                        <h2 className='text-[10px] bg-green-100 p-1 rounded-full
                        px-2 text-primary'>{doctor.attributes?.categories.data[0].attributes?.Name}</h2>
                        <h2 className='font-bold'>{doctor.attributes.Name}</h2>
                        <h2 className='text-primary text-sm'>{doctor.attributes?.Year_of_Experience} + years of Experience</h2>
